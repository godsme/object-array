...
namespace mixin {
    ...
    struct ObjectCompareExt : T {
        ...
        auto GetGreater() const -> auto {
            if constexpr(std::is_rvalue_reference_v<decltype(T::GetLess())>) {
                return [&less = T::GetLess()](ObjectType const &l, ObjectType const &r) { return less(r, l); };
            } else {
                return [less = T::GetLess()](ObjectType const &l, ObjectType const &r) { return less(r, l); };
            }
        }
